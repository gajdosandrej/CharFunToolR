## EXAMPLE1
# CF of the uniform von Mises distribution on (-pi,pi)
...
## EXAMPLE3
# PDF/CDF of the von Mises distribution on (-pi,pi)
mu <- 0
kappa <- 5
cf  <- function(t)
...
## EXAMPLE4
# PDF/CDF of the linear combinantion of 2 von Mises distribution on (-pi,pi)
mu1 <- 0
kappa1 <- 5
mu2 <- 1
kappa2 <- 15
...
cf <-
        function(t)
                0.25 * cfC_vonMises(t, mu1, kappa1) + 0.25 * cfC_vonMises(t, mu2, kappa2) + 0.5 * cfC_vonMises(t, mu3, kappa3)
options <- list()
options$xMin <- 0
options$xMax <- 2 * pi
options$isCircular <- TRUE
