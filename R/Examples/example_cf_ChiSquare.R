...
plotGraf(function(t)
  cf_ChiSquare(t,df,ncp,coef), t, title = expression('Characteristic function of the' ~ chi^2 ~ 'RV with DF=1 and NCP=1'))
...
t <- seq(from = -100,to = 100,length.out = 501)
plotGraf(function(t)
  cf_ChiSquare(t=t,df=df,coef=coef), t, title = expression('CF of the linear combination of' ~ chi^2 ~ 'RVs with DF=1'))

## EXAMPLE 3
# PDF/CDF from the CF by cf2DistGP
df <- 1
coef <-  1/(((1:50) - 0.5)*pi)^2
cf <- function(t) cf_ChiSquare(t=t,df=df,coef=coef)
options <- list()
options$N <- 2^12
options$xMin <- 0
x <- seq(from = 0,to = 3.5,length.out = 501)
